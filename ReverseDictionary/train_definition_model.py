...
tf.app.flags.DEFINE_integer("num_epochs", 1, "Train for this number of"
...
tf.app.flags.DEFINE_boolean("restore_and_query", False, "Restore a trained model"
                            "instead of training one.")
  ...
  with tf.device("/device:GPU:0"):
    ...
    if pretrained_target:
      ...
      if pretrained_input:
        ...
        # Target embeddings are pre-trained. out_embd_matrix is 100000*300
  ...
  # Load an existing model.
    ...
    if FLAGS.restore_and_query:
        ...
        with sess:
            ...
            for idx, epoch in enumerate(
                    gen_epochs(
                        FLAGS.data_dir, FLAGS.num_epochs, FLAGS.batch_size, FLAGS.vocab_size, phase="train")):
                # Running total for training loss reset every 500 steps.
                training_loss = 0
                print("\nEPOCH", idx)
                for step, (gloss, head) in enumerate(epoch):
                ...
                # Save current model after another epoch.
                if idx % 50 == 0:
                    save_path = os.path.join(FLAGS.save_dir, "%s_%s.ckpt" % (FLAGS.model_name, idx))
                    save_path = saver.save(sess, save_path)
                    print("Model saved in file: %s after epoch: %s" % (save_path, idx))
            print("Total data points seen during training: %s" % num_training)
